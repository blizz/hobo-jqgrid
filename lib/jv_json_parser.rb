...
#
# Parse code from: http://rubyquiz.com/quiz155.html
# modified slightly
#
...
class JvJsonParser
  ...
  def parse_string
    string = String.new
    while contents = parse_string_nq_content || parse_string_escape
  ...
  def parse_qt_string
    if @input.scan(/"/)
      ...
      if @input.scan(/'/)
        string = String.new
        while contents = parse_string_sq_content || parse_string_escape
          string << contents.value
        end
        @input.scan(/'/) or error("Unclosed string")
        AST.new(string)
  ...
  def parse_string_sq_content
    @input.scan(/[^\\']+/) and AST.new(@input.matched)
  end
  
  def parse_string_nq_content
    @input.scan(/[^\\:},\]]+/) and AST.new(@input.matched)
  end
  ...
  def parse_number
    @input.scan(/-?(?:0|[1-9]\d*)(?:\.\d+)?(?:[eE][+-]?\d+)?\b/) and AST.new(eval(@input.matched))
  end

  def parse_keyword
    @input.scan(/\b(?:true|false|null)\b/) and AST.new(eval(@input.matched.sub("null", "nil")))
  end  
